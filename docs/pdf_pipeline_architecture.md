...
| **P3** ✅（2026-07-31 完成，289 单测全过，分层评测全指标达标）| L1 视觉理解路径（figure 区域渲染 → 多模态描述回填）；L4 Agent 工具层全套（五工具 + 规则路由 + 步数兜底 + 自动回退）；分层评测集（离线可跑） | +多模态模型 API（可选，不可用自动降级） | 图表问题走 analyze_chart，表格问题走 extract_table，复杂对比多步检索 |
