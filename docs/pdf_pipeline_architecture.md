...
| 阶段 | 内容 | 依赖变化 | 验收 |
|------|------|----------|------|
| **P0** | L1 块级抽取 + 页面分类器；L2 页眉页脚过滤 + 多栏重组 + TOC 提取；L3 结构 metadata 入库（page/section/kind） | 无新依赖 | 合同 PDF 切片带"第几章第几条第几页" |
| **P1** | L2 表格结构化（find_tables）+ 条款边界；L3 语义切片器取代硬切；引用带页码 | 无新依赖 | 财报表格 chunk 行列完整，引用可跳页 |
