...
- 统计卡片、列表容器 → `card`（实施裁量：上传拖拽区保持虚线透明底，不用 `card`——与本节拖拽区规则冲突时以拖拽区规则为准）
...
- AI 气泡 `bg-white border border-gray-200` → 内联 `bg-surface-raised border border-line`（实施裁量：保留 `rounded-2xl` 异形圆角，不用 `card`——`card` 的 rounded-theme + 2px 墨框会破坏气泡造型；editorial 下 1px 墨框仍成立）
...
**执行记录（终审后补记）：**
- 截图核对发现「单页时空分页容器在 editorial 下显示为突兀墨线」，已修复（`KBListPage.tsx` 加分页条件，commit f396b73）
- 对比度三项决策（editorial accent ≈4.2:1、`bg-accent/10 text-accent` ≈4:1、th/td 不对称）：**保持现状**——该配色在头脑风暴阶段经用户目视批准，内部工具可接受；如未来有无障碍要求，候选动作为 editorial accent 压暗至 `#c23423`
