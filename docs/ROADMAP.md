...
> **项目状态**: 全部版本已交付，Sprint 8 收尾任务 100% 完成（125/125），v2.1.0 已发布
...
v1.5 增强版                           v2.0 企业版
  Sprint 9-12 (提前交付)               Sprint 13-16 (提前交付)
      ✅                                     ✅
```
...
| 版本 | Sprint | 状态 | 核心目标 |
|------|--------|:---:|------|
| v0.1 | 1-2 | ✅ 已交付 | 项目初始化 + 核心基础设施 + 用户认证 |
| v0.2 | 3-4 | ✅ 已交付 | 文档上传 → 解析 → 分块 → 向量化 → Qdrant |
| v0.3 | 5 | ✅ 已交付 | 查询改写 + 向量/BM25/混合检索 + 重排序 |
| v0.4 | 6-7 | ✅ 已交付 | LLM 调用 + Prompt + 流式 + 引用 + 多轮对话 |
| **v1.0 MVP** | 8 | ✅ 已发布 | 前端集成 + Docker 部署 + 文档 |
| v1.5 | 9-12 | ✅ 提前交付 | 混合检索 + RBAC + 多轮对话 + 反馈 + Word解析 + 批量上传 |
| v2.0 | 13-16 | ✅ 提前交付 | 跨知识库 + 分析面板 + SSO + API Key + K8s |

**当前进度：全部功能与收尾任务 100% 完成，v2.1.0 已发布（2026-07-31）。**
...
### v1.5 增强版（Sprint 9-12，第 17-24 周）— ✅ 已提前交付

| Sprint | 主题 | 核心交付物 | 状态 |
|--------|------|-----------|:---:|
| 9 | 混合检索 | BM25 索引（PostgreSQL tsvector + jieba）、RRF 融合、HybridRetriever | ✅ |
...
### v2.0 企业版（Sprint 13-16，第 25-32 周）— ✅ 已提前交付

| Sprint | 主题 | 核心交付物 | 状态 |
|--------|------|-----------|:---:|
| 13 | 跨知识库 | 跨知识库检索、知识库关联、统一检索入口 | ✅ |
| 14 | 数据分析 | 数据看板（问答趋势/热门问题/用户活跃度）、图表可视化 | ✅ |
| 15 | SSO + API | OIDC 单点登录、开放 API + API Key 管理、Rate Limiting | ✅ |
| 16 | 生产就绪 | K8s Helm Chart、性能压测、安全审计、v2.0 Release | ✅ |

---

## 5. 里程碑节点

| 里程碑 | Sprint | 时间 | 验收标准 | 状态 |
|--------|--------|------|---------|:---:|
...
| **M5: 检索可用** | Sprint 5 | 第 10 周末 | 检索链路完整，Recall@5 ≥ 90%（实际 100%） | ✅ |
...
| **M9: v1.5 增强版** | Sprint 9-12 | 提前交付 | 混合检索 + RBAC + 多轮 + 反馈 + Word + 批量上传 | ✅ |
| **M10: v2.0 企业版** | Sprint 13-16 | 提前交付 | 跨知识库 + 数据看板 + SSO + API Key + K8s | ✅ |
...
## 7. Sprint 8 任务清单 — 全部完成 ✅

| ID | 任务 | 优先级 | 状态 | 说明 |
|----|------|:---:|:---:|------|
| 8.1 | 前后端全链路联调 | P0 | ✅ | 12 个 P0 User Story 验证通过：`scripts/e2e_smoke.py` 18/18 项全绿（注册/登录/建库/上传/处理/问答/引用/流式/多轮/删除同步/监控） |
| 8.4 | Docker 镜像构建验证 | P0 | ✅ | rag-api:latest（8.9GB）+ rag-worker:latest 均构建成功并可导入运行；Worker 镜像改为共享 API 依赖层，构建秒级完成、磁盘占用减半 |
| 8.10 | 文档一致性审核 | P1 | ✅ | PRD/ARCH/API/DATABASE 一致性核验完成，ROADMAP + TASKS 已更新 |
| 8.12 | Git Tag + Release Notes | P1 | ✅ | v2.1.0 Tag 已创建并推送，CHANGELOG 定版 |
